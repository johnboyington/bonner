...
def imp_spheres_mcnpWriter(size, energy, particles):
    s = open('card0.txt', 'r').read()
    for spheres in range(7):
        s += '{}  SO  {}    \n'.format(spheres + 1, size * ((spheres + 1) / 7))
    s += open('card1.txt', 'r').read()
    s += 'NPS   {}\n'.format(particles)
    s += 'c  ---------------------------------------------------------\n'
    s += 'c                    SOURCE SPECIFICATIONS\n'
...
def runFile3(size, energy, particles):
    imp_spheres_mcnpWriter(size * (2.54 / 2), energy, particles)
    os.chdir('working')
    os.system('mcnp6 inp=inp tasks 27')
    response = Extract()
    os.remove('inp')
...
def checkGeo(size, energy, particles):
    imp_spheres_mcnpWriter(size * (2.54 / 2), energy, particles)
    os.chdir('working')
    os.system('mcnp6 rp inp=inp')
    return
